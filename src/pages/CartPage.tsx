            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center">
                ...
                <div className="flex-1 ml-3 -mt-3 -mr-10">
                  ...
                  <p className="font-bold text-[18px] mt-1">${item.price}</p>
                </div>
                <button className='-mt-10' onClick={() => removeFromCart(item.id)}>
                  <MdDelete size={25} className='fill-[#A435F0] hover:fill-[#8710D8]' />
                </button>
              </div>
            ))}
